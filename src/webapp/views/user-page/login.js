    ...
    if($("#sign-up").hasClass("active")) {
        ...
        if(check1&&check2&&check3&&check4) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    window.location.replace("./login.php?msg=Đăng ký thành công&alert=success")
                    $("#log-in").click()
                },
                error: function(jqXHR, exception) {
                    window.location.replace("./login.php?msg=Vui lòng thử lại!&alert=danger")
                }
            });
        } else
            e.preventDefault()
        ...
        if(checkLogin1&&checkLogin2) {
            ...
            $.ajax({
                ...
                error: function(jqXHR, exception) {
                    window.location.replace("./login.php?msg=Sai tên tài khoản hoặc mật khẩu!&alert=danger")
                }
            });
        } else
            e.preventDefault()
